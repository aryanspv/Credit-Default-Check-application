...
def main():
    ...
    BILL_AMT1 = st.text_input("Last month Bill Amount (in $)")
    BILL_AMT2 = st.text_input("Last 2nd month Bill Amount (in $)")
    BILL_AMT3 = st.text_input("Last 3rd month Bill Amount (in $)")
    BILL_AMT4 = st.text_input("Last 4th month Bill Amount (in $)")
    BILL_AMT5 = st.text_input("Last 5th month Bill Amount (in $)")
    BILL_AMT6 = st.text_input("Last 6th month Bill Amount (in $)")

    PAY_AMT1 = st.text_input("Amount paid in Last Month (in $)")
    PAY_AMT2 = st.text_input("Amount paid in Last 2nd month (in $)")
    PAY_AMT3 = st.text_input("Amount paid in Last 3rd month (in $)")
    PAY_AMT4 = st.text_input("Amount paid in Last 4th month (in $)")
    PAY_AMT5 = st.text_input("Amount paid in Last 5th month (in $)")
    PAY_AMT6 = st.text_input("Amount paid in Last 6th month (in $)")
    ...
    if st.button("Predict"):
        ...
        if prediction[0] == 1:
            ...
            #         <p style = 'color: #4eb5de;'>This account will be defaulted with a probability of {round(np.max(probability)*100, 2))}%.</p>
            #     </div>
            # """
